...
export default function ProductCard({ product, onRename, onChangeQty, onSetQty, onChangePhoto, onDelete }) {
  ...
  function commitName() { const t = name.trim(); if (!t) { setName(product.name); return; } if (t !== product.name) onRename(product.id, t); }
  const askQtyInput = (text) => { const n = parseInt(text.replace(/[^0-9]/g, ''), 10); onSetQty(product.id, isNaN(n) ? 0 : n); };
  ...
  return (
    <CardShell cat={product.cat} photo={product.photo}
      onPhotoPress={() => setPickerVisible(true)}
      overlayLabel={product.photo ? 'Cambiar' : 'Añadir foto'} onOverlayPress={() => setPickerVisible(true)}
      nameNode={(
        <View style={st.nameRow}>
          <TextInput style={st.nameInput} value={name} onChangeText={setName} onEndEditing={commitName} onSubmitEditing={commitName} returnKeyType="done" numberOfLines={1} scrollEnabled placeholder="Nombre" placeholderTextColor={colors.steel} />
          <TouchableOpacity onPress={() => setConfirmVisible(true)} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}><IconBasura color={colors.steel} /></TouchableOpacity>
        </View>
      )}>
      ...
      <PhotoChooserPanel title="Cambiar foto" visible={pickerVisible} onClose={() => setPickerVisible(false)} onPickCamera={handlePickCamera} onPickLibrary={handlePickLibrary} onPickWeb={uri => onChangePhoto(product.id, uri)} initialQuery={product.name} />
      <ConfirmModal visible={confirmVisible} title="Eliminar producto" message={`¿Eliminar "${product.name}" del depósito? Esta acción no se puede deshacer.`} onCancel={() => setConfirmVisible(false)} onConfirm={() => { setConfirmVisible(false); onDelete(product.id); }} />
    </CardShell>
  );
}
